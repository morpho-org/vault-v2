...
library VaultV2AddressLib {
    ...
    function computeVaultV2Address(address lens, address factory, address owner, address asset, bytes32 salt)
        internal
        pure
        returns (address)
    {
        bytes32 initCodeHash = keccak256(abi.encodePacked(type(VaultV2).creationCode, abi.encode(lens, owner, asset)));
