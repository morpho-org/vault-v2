...
library UtilsLib {
    ...
    /// @dev Returns the first word returned by a successful call.
    /// @dev Returns 0 if no data was returned.
    /// @dev Returns 0 if the call reverted.
    function controlledCall(address to, bytes memory data) internal returns (uint256) {
        uint256[1] memory output;
        bool success;
        assembly ("memory-safe") {
            success := call(gas(), to, 0, add(data, 32), mload(data), output, 32)
        }
        if (success) {
            return output[0];
        } else {
