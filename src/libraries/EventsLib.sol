...
library EventsLib {
    ...
    event IncreaseAbsoluteCap(bytes id, uint256 newAbsoluteCap);
    event DecreaseAbsoluteCap(bytes id, uint256 newAbsoluteCap);
    event IncreaseRelativeCap(bytes id, uint256 newRelativeCap);
    event DecreaseRelativeCap(bytes id, uint256 newRelativeCap);
    event SetForceReallocateToIdlePenalty(uint256 forceReallocateToIdleFee);
    event ReallocateFromIdle(address indexed sender, address indexed adapter, uint256 assets, bytes[] ids);
    event ReallocateToIdle(address indexed sender, address indexed adapter, uint256 assets, bytes[] ids);
