...
contract VaultV2 is IVaultV2 {
    ...
    function accrueInterestView() public view returns (uint256, uint256, uint256) {
        ...
        int256 change = int256(uint256(_totalAssets)) - int256(realAssets);
        uint256 maxInterest = (_totalAssets * elapsed).mulDivDown(maxRate, WAD);
        uint256 newTotalAssets =
            MathLib.min(uint256(int256(uint256(_totalAssets)) + change), _totalAssets + maxInterest);
        ...
        if (change > 0 && performanceFee > 0 && canReceiveShares(performanceFeeRecipient)) {
            performanceFeeAssets = uint256(change).mulDivDown(performanceFee, WAD);
        }
