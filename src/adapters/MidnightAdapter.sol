...
contract MidnightAdapter is IMidnightAdapter {
    ...
    function onSell(
        ...
        if (block.timestamp < market.maturity) {
            _marketData[marketId].allowance -= saleShortfall.toUint128();
        }
        ...
        returns (int256 change)
    {
        ...
        data.allowance = MathLib.min(
                allowanceCap,
                data.allowance + allowanceCap.mulDivDown(block.timestamp - data.updatedAt, SHORTFALL_REFILL_PERIOD)
            )
            .toUint128();
        data.updatedAt = block.timestamp.toUint48();
        ...
        emit UpdateMarket(marketId, data);
