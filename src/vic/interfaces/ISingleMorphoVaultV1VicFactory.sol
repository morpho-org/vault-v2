...
interface ISingleMorphoVaultV1VicFactory {
    ...
    event CreateSingleMorphoVaultV1Vic(
        address indexed vic, address indexed parentVault, address indexed morphoVaultV1Adapter
    );

    /* FUNCTIONS */

    function isSingleMorphoVaultV1Vic(address account) external view returns (bool);
    function singleMorphoVaultV1Vic(address parentVault, address morphoVaultV1Adapter)
        external
        view
        returns (address);
