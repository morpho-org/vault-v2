...
/// funds.
/// @dev If account is registered as a trusted intermediary, IIntermediary(account).initiator() is checked for whitelisted status
/// instead of account itself. Thus the intermediary should only deposit and mint using assets initially owned by its current initiator.
