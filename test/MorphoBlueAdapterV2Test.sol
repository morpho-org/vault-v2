...
import {MorphoBlueAdapterV2} from "../src/adapters/MorphoBlueAdapterV2.sol";
import {MorphoBlueAdapterV2Factory} from "../src/adapters/MorphoBlueAdapterV2Factory.sol";
...
import {IMorphoBlueAdapterV2} from "../src/adapters/interfaces/IMorphoBlueAdapterV2.sol";
import {IMorphoBlueAdapterV2Factory} from "../src/adapters/interfaces/IMorphoBlueAdapterV2Factory.sol";
import {MathLib} from "../src/libraries/MathLib.sol";
import {IAdaptiveCurveIrm} from "../lib/morpho-blue-irm/src/adaptive-curve-irm/interfaces/IAdaptiveCurveIrm.sol";

contract MorphoBlueAdapterV2Test is Test {
    using MorphoBalancesLib for IMorpho;
    using MarketParamsLib for MarketParams;
    using MathLib for uint256;

    IMorphoBlueAdapterV2Factory internal factory;
    IMorphoBlueAdapterV2 internal adapter;
    ...
    function setUp() public {
        ...
        factory = new MorphoBlueAdapterV2Factory(address(morpho), address(irm));
        adapter = IMorphoBlueAdapterV2(factory.createMorphoBlueAdapterV2_005kLDJ(address(parentVault)));
        ...
        vm.prank(curator);
        adapter.submit(abi.encodeCall(IMorphoBlueAdapterV2.setSkimRecipient, (recipient)));
    ...
    function testAllocateNotAuthorizedReverts(uint256 assets) public {
        ...
        vm.expectRevert(IMorphoBlueAdapterV2.Unauthorized.selector);
    ...
    function testAllocateDifferentAssetReverts(address randomAsset, uint256 assets) public {
        ...
        vm.expectRevert(IMorphoBlueAdapterV2.LoanAssetMismatch.selector);
    ...
    function testFactorycreateMorphoBlueAdapterV2_005kLDJ() public {
        address newParentVaultAddr =
            address(new VaultV2Mock(address(loanToken), owner, address(0), address(0), address(0)));

        bytes32 initCodeHash = keccak256(
            abi.encodePacked(
                type(MorphoBlueAdapterV2).creationCode, abi.encode(newParentVaultAddr, address(morpho), address(irm))
            )
        );
        address expectedNewAdapter =
            address(uint160(uint256(keccak256(abi.encodePacked(uint8(0xff), factory, bytes32(0), initCodeHash)))));
        vm.expectEmit();
        emit IMorphoBlueAdapterV2Factory.CreateMorphoMarketV1AdapterV2(newParentVaultAddr, expectedNewAdapter);

        address newAdapter = factory.createMorphoBlueAdapterV2_005kLDJ(newParentVaultAddr);

        expectedIds[0] = keccak256(abi.encode("this", address(newAdapter)));

        assertTrue(newAdapter != address(0), "Adapter not created");
        assertEq(IMorphoBlueAdapterV2(newAdapter).factory(), address(factory), "Incorrect factory");
        assertEq(IMorphoBlueAdapterV2(newAdapter).parentVault(), newParentVaultAddr, "Incorrect parent vault");
        assertEq(IMorphoBlueAdapterV2(newAdapter).asset(), address(loanToken), "Incorrect asset");
        assertEq(IMorphoBlueAdapterV2(newAdapter).morpho(), address(morpho), "Incorrect morpho");
        assertEq(IMorphoBlueAdapterV2(newAdapter).adapterId(), expectedIds[0], "Incorrect adapterId");
        assertEq(factory.morphoBlueAdapterV2_71Aurmp(newParentVaultAddr), newAdapter, "Adapter not tracked correctly");
        assertTrue(factory.isMorphoBlueAdapterV2_60omp0Z(newAdapter), "Adapter not tracked correctly");
    }

    function testSetSkimRecipientNotTimelocked(address newRecipient) public {
        vm.assume(newRecipient != address(0));

        vm.expectRevert(IMorphoBlueAdapterV2.DataNotTimelocked.selector);
    ...
    function testSetSkimRecipientNotAuthorized(address newRecipient, address caller) public {
        ...
        vm.expectRevert(IMorphoBlueAdapterV2.Unauthorized.selector);
        vm.prank(caller);
        adapter.submit(abi.encodeCall(IMorphoBlueAdapterV2.setSkimRecipient, (newRecipient)));
    }
    ...
    function testSetSkimRecipientTimelockNotExpired(address newRecipient, uint256 timelockDuration) public {
        ...
        adapter.submit(
            abi.encodeCall(
                IMorphoBlueAdapterV2.increaseTimelock,
                (IMorphoBlueAdapterV2.setSkimRecipient.selector, timelockDuration)
            )
        );
        adapter.increaseTimelock(IMorphoBlueAdapterV2.setSkimRecipient.selector, timelockDuration);

        vm.prank(curator);
        adapter.submit(abi.encodeCall(IMorphoBlueAdapterV2.setSkimRecipient, (newRecipient)));

        vm.expectRevert(IMorphoBlueAdapterV2.TimelockNotExpired.selector);
        adapter.setSkimRecipient(newRecipient);
    }
    ...
    function testSetSkimRecipient(address newRecipient, uint256 timelockDuration) public {
        ...
        vm.expectEmit();
        emit IMorphoBlueAdapterV2.SetSkimRecipient(newRecipient);
        adapter.setSkimRecipient(newRecipient);
    ...
    function testSkim(uint256 assets) public {
        ...
        vm.expectEmit();
        emit IMorphoBlueAdapterV2.Skim(address(token), assets);
        vm.prank(recipient);
    ...
    function testSubmitBurnSharesNotAuthorized(address caller, bytes32 _marketId) public {
        ...
        vm.prank(caller);
        vm.expectRevert(IMorphoBlueAdapterV2.Unauthorized.selector);
        adapter.submit(abi.encode(IMorphoBlueAdapterV2.burnShares.selector, abi.encode(_marketId)));
    }

    function testSubmitBurnSharesAlreadyPending(bytes32 _marketId) public {
        vm.prank(curator);
        adapter.submit(abi.encodeCall(IMorphoBlueAdapterV2.burnShares, (_marketId)));

        vm.expectRevert(IMorphoBlueAdapterV2.DataAlreadyPending.selector);
        vm.prank(curator);
        adapter.submit(abi.encodeCall(IMorphoBlueAdapterV2.burnShares, (_marketId)));
    }
    ...
    function testSubmitBurnShares(bytes32 _marketId, uint256 timelockDuration) public {
        ...
        adapter.submit(
            abi.encodeCall(
                IMorphoBlueAdapterV2.increaseTimelock, (IMorphoBlueAdapterV2.burnShares.selector, timelockDuration)
            )
        );
        adapter.increaseTimelock(IMorphoBlueAdapterV2.burnShares.selector, timelockDuration);
        vm.expectEmit();
        emit IMorphoBlueAdapterV2.Submit(
            IMorphoBlueAdapterV2.burnShares.selector,
            abi.encodeCall(IMorphoBlueAdapterV2.burnShares, (_marketId)),
            block.timestamp + adapter.timelock(IMorphoBlueAdapterV2.burnShares.selector)
        );
        ...
        assertEq(
            adapter.executableAt(abi.encodeCall(IMorphoBlueAdapterV2.burnShares, (_marketId))),
            block.timestamp + adapter.timelock(IMorphoBlueAdapterV2.burnShares.selector)
        );
    }
    ...
    function testRevokeBurnSharesNotAuthorized(address caller, bytes32 _marketId) public {
        ...
        vm.prank(caller);
        vm.expectRevert(IMorphoBlueAdapterV2.Unauthorized.selector);
        adapter.revoke(abi.encodeCall(IMorphoBlueAdapterV2.burnShares, (_marketId)));
    }
    ...
    function testRevokeBurnShares(bytes32 _marketId) public {
        ...
        vm.prank(curator);
        vm.expectEmit();
        emit IMorphoBlueAdapterV2.Revoke(
            curator,
            IMorphoBlueAdapterV2.burnShares.selector,
            abi.encodeCall(IMorphoBlueAdapterV2.burnShares, (_marketId))
        );
        ...
        assertEq(adapter.executableAt(abi.encodeCall(IMorphoBlueAdapterV2.burnShares, (_marketId))), 0);
    ...
    function testBurnShares(uint256 timelockDuration, uint256 extraSkip) public {
        ...
        vm.prank(curator);
        adapter.submit(abi.encodeCall(IMorphoBlueAdapterV2.burnShares, (marketId)));

        skip(timelockDuration + bound(extraSkip, 0, 3650 days));

        vm.expectEmit();
        emit IMorphoBlueAdapterV2.BurnShares(marketId, supplyShares);
        ...
        assertEq(adapter.executableAt(abi.encodeCall(IMorphoBlueAdapterV2.burnShares, (marketId))), 0, "executable at");
        assertEq(adapter.realAssets(), 0, "realAssets");
    }

    function testAbdicated() public {
        vm.prank(curator);
        adapter.submit(abi.encodeCall(IMorphoBlueAdapterV2.abdicate, (IMorphoBlueAdapterV2.burnShares.selector)));
        adapter.abdicate(IMorphoBlueAdapterV2.burnShares.selector);

        vm.prank(curator);
        adapter.submit(abi.encodeCall(IMorphoBlueAdapterV2.burnShares, (marketId)));

        vm.expectRevert(IMorphoBlueAdapterV2.Abdicated.selector);
        vm.prank(curator);
        adapter.burnShares(marketId);
    }
}
