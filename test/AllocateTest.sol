...
contract AllocateTest is BaseTest {
        ...
        public
    {
        ...
        vm.prank(allocator);
        vm.expectEmit();
        emit EventsLib.Allocate(allocator, adapter, data, allocateAssets, ids, 0);
        ...
        public
    {
        ...
        vm.prank(allocator);
        vm.expectEmit();
        emit EventsLib.Deallocate(allocator, adapter, data, assetsOut, ids, 0);
    ...
    function testAllocateWithInterest(
        ...
        if (cap >= allocation1 + allocation2 + interest) {
            vm.expectEmit();
            emit EventsLib.Allocate(allocator, adapter, hex"", allocation2, ids, interest);
