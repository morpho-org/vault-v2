...
contract AllocateTest is BaseTest {
    ...
    function testAllocate(bytes memory data, uint256 assets, address rdm, uint256 absoluteCap, uint256 profit) public {
        ...
        vm.prank(allocator);
        vm.expectEmit();
        emit EventsLib.Allocate(allocator, mockAdapter, assets, ids, int256(assets + profit));
        vault.allocate(mockAdapter, data, assets);
    ...
    function testDeallocate(
        ...
        vm.prank(allocator);
        vm.expectEmit();
        emit EventsLib.Deallocate(allocator, mockAdapter, assetsOut, ids, -int256(assetsOut) + int256(profit));
