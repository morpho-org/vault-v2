...
contract SettersTest is BaseTest {
    ...
    function testsetLiquidityAdapter(address rdm, address liquidityAdapter, bytes memory liquidityData) public {
        ...
        vm.expectRevert(ErrorsLib.Unauthorized.selector);
        vm.prank(rdm);
        vault.setLiquidityAdapter(liquidityAdapter, liquidityData);
        ...
        vm.prank(allocator);
        vm.expectEmit();
        emit EventsLib.SetLiquidityAdapter(allocator, liquidityAdapter, liquidityData);
        vault.setLiquidityAdapter(liquidityAdapter, liquidityData);
