...
contract MMDonationAttackTest is Test {
    ...
    function testLossByDeposit1() public {
        // Seed
        ...
        assertApproxEqRel(
            metaMorpho.previewRedeem(SEED), SEED * HIGH_SHARE_PRICE, WAD / 1e3, "share price after donation"
        );
        ...
        while (eveBalance() < FLASHLOAN) {
            ...
            if (true || iterations == limit) {
                ...
                console.log("mm total assets %s", metaMorpho.totalAssets());
                console.log("mm last total assets %s", metaMorpho.lastTotalAssets());
            }
            if (FLASHLOAN > eveBalance()) {
                console.log("eve loss      %s", FLASHLOAN - eveBalance());
            } else {
                console.log("eve gain      %s", eveBalance() - FLASHLOAN);
            }
            ...
            // if (metaMorpho.lastTotalAssets() > type(uint184).max/2) {
            //     metaMorpho.withdraw(vault.previewRedeem(vault.balanceOf(eve))/2,address(this),address(this));
            // }
