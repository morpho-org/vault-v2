...
contract MorphoSingleMarketV1AdapterTest is Test {
    ...
    function setUp() public {
        ...
        adapter = MorphoSingleMarketV1Adapter(
            factory.createMorphoSingleMarketV1Adapter(address(parentVault), address(morpho), marketParams)
        );
    ...
    function testAllocateNotAuthorizedReverts(uint256 assets) public {
        ...
        vm.expectRevert(IMorphoSingleMarketV1Adapter.NotAuthorized.selector);
    ...
    function testLoanAssetMismatchReverts() public {
        ...
        vm.expectRevert(IMorphoSingleMarketV1Adapter.LoanAssetMismatch.selector);
        new MorphoSingleMarketV1Adapter(address(parentVault), address(morpho), mismatchMarketParams);
    }

    function testFactoryCreateMorphoSingleMarketV1Adapter() public {
        address newParentVaultAddr =
            address(new VaultV2Mock(address(loanToken), owner, address(0), address(0), address(0)));

        bytes32 initCodeHash = keccak256(
            abi.encodePacked(
                type(MorphoSingleMarketV1Adapter).creationCode, abi.encode(newParentVaultAddr, morpho, marketParams)
            )
        );
        address expectedNewAdapter =
            address(uint160(uint256(keccak256(abi.encodePacked(uint8(0xff), factory, bytes32(0), initCodeHash)))));
        vm.expectEmit();
        emit IMorphoSingleMarketV1AdapterFactory.CreateMorphoSingleMarketV1Adapter(
        ...
        assertEq(IMorphoSingleMarketV1Adapter(newAdapter).factory(), address(factory), "Incorrect factory");
        assertEq(IMorphoSingleMarketV1Adapter(newAdapter).parentVault(), newParentVaultAddr, "Incorrect parent vault");
        assertEq(IMorphoSingleMarketV1Adapter(newAdapter).morpho(), address(morpho), "Incorrect morpho");
        assertEq(IMorphoSingleMarketV1Adapter(newAdapter).adapterId(), expectedIds[0], "Incorrect adapterId");
        assertEq(
            factory.morphoSingleMarketV1Adapter(newParentVaultAddr, address(morpho), marketId),
            newAdapter,
            "Adapter not tracked correctly"
        );
        MarketParams memory returnedMarketParams = IMorphoSingleMarketV1Adapter(newAdapter).marketParams();
    ...
    function testSetSkimRecipient(address newRecipient, address caller) public {
        ...
        vm.prank(owner);
        vm.expectEmit();
        emit IMorphoSingleMarketV1Adapter.SetSkimRecipient(newRecipient);
        adapter.setSkimRecipient(newRecipient);
    ...
    function testSkim(uint256 assets) public {
        ...
        vm.expectEmit();
        emit IMorphoSingleMarketV1Adapter.Skim(address(token), assets);
        vm.prank(recipient);
