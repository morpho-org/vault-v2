...
import {IBlueAdapterV3Factory} from "../src/adapters/interfaces/IBlueAdapterV3Factory.sol";
import {MathLib} from "../src/libraries/MathLib.sol";
import {IAdaptiveCurveIrm} from "../lib/morpho-blue-irm/src/adaptive-curve-irm/interfaces/IAdaptiveCurveIrm.sol";

contract BlueAdapterV3Test is Test {
    using MorphoBalancesLib for IMorpho;
    using MarketParamsLib for MarketParams;
    using MathLib for uint256;

    IBlueAdapterV3Factory internal factory;
    IBlueAdapterV3 internal adapter;
    ...
    function setUp() public {
        ...
        factory = new BlueAdapterV3Factory(address(morpho));
        adapter = IBlueAdapterV3(factory.createBlueAdapterV3(address(parentVault)));
        ...
        vm.prank(curator);
        adapter.submit(abi.encodeCall(IBlueAdapterV3.setSkimRecipient, (recipient)));
    ...
    function testAllocateNotAuthorizedReverts(uint256 assets) public {
        ...
        vm.expectRevert(IBlueAdapterV3.Unauthorized.selector);
    ...
    function testAllocateDifferentAssetReverts(address randomAsset, uint256 assets) public {
        ...
        vm.expectRevert(IBlueAdapterV3.LoanAssetMismatch.selector);
    ...
    function testAllocateSupportsArbitraryIrm(uint256 assets) public {
        assets = _boundAssets(assets);
        IrmMock otherIrm = new IrmMock();
        MarketParams memory otherMarketParams = marketParams;
        otherMarketParams.irm = address(otherIrm);

        vm.prank(morpho.owner());
        morpho.enableIrm(address(otherIrm));
        morpho.createMarket(otherMarketParams);

        deal(address(loanToken), address(adapter), assets);
        (bytes32[] memory ids, int256 change) =
            parentVault.allocateMocked(address(adapter), abi.encode(otherMarketParams), assets);

        bytes32 otherMarketId = Id.unwrap(otherMarketParams.id());
        assertEq(change, int256(assets), "Incorrect change returned");
        assertEq(morpho.expectedSupplyAssets(otherMarketParams, address(adapter)), assets, "Incorrect assets in Morpho");
        assertEq(adapter.marketIds(0), otherMarketId, "Incorrect market id");
        assertEq(ids[2], keccak256(abi.encode("this/marketParams", address(adapter), otherMarketParams)));
    }
    ...
    function testFactoryCreateBlueAdapterV3() public {
        address newParentVaultAddr =
            address(new VaultV2Mock(address(loanToken), owner, address(0), address(0), address(0)));

        bytes32 initCodeHash = keccak256(
            abi.encodePacked(type(BlueAdapterV3).creationCode, abi.encode(newParentVaultAddr, address(morpho)))
        );
        address expectedNewAdapter =
            address(uint160(uint256(keccak256(abi.encodePacked(uint8(0xff), factory, bytes32(0), initCodeHash)))));
        vm.expectEmit();
        emit IBlueAdapterV3Factory.CreateBlueAdapterV3(newParentVaultAddr, expectedNewAdapter);

        address newAdapter = factory.createBlueAdapterV3(newParentVaultAddr);

        expectedIds[0] = keccak256(abi.encode("this", address(newAdapter)));

        assertTrue(newAdapter != address(0), "Adapter not created");
        assertEq(IBlueAdapterV3(newAdapter).factory(), address(factory), "Incorrect factory");
        assertEq(IBlueAdapterV3(newAdapter).parentVault(), newParentVaultAddr, "Incorrect parent vault");
        assertEq(IBlueAdapterV3(newAdapter).asset(), address(loanToken), "Incorrect asset");
        assertEq(IBlueAdapterV3(newAdapter).morpho(), address(morpho), "Incorrect morpho");
        assertEq(IBlueAdapterV3(newAdapter).adapterId(), expectedIds[0], "Incorrect adapterId");
        assertEq(factory.blueAdapterV3(newParentVaultAddr), newAdapter, "Adapter not tracked correctly");
        assertTrue(factory.isBlueAdapterV3(newAdapter), "Adapter not tracked correctly");
    }

    function testSetSkimRecipientNotTimelocked(address newRecipient) public {
        vm.assume(newRecipient != address(0));

        vm.expectRevert(IBlueAdapterV3.DataNotTimelocked.selector);
    ...
    function testSetSkimRecipientNotAuthorized(address newRecipient, address caller) public {
        ...
        vm.expectRevert(IBlueAdapterV3.Unauthorized.selector);
        vm.prank(caller);
        adapter.submit(abi.encodeCall(IBlueAdapterV3.setSkimRecipient, (newRecipient)));
    }
    ...
    function testSetSkimRecipientTimelockNotExpired(address newRecipient, uint256 timelockDuration) public {
        ...
        adapter.submit(
            abi.encodeCall(
                IBlueAdapterV3.increaseTimelock, (IBlueAdapterV3.setSkimRecipient.selector, timelockDuration)
            )
        );
        adapter.increaseTimelock(IBlueAdapterV3.setSkimRecipient.selector, timelockDuration);

        vm.prank(curator);
        adapter.submit(abi.encodeCall(IBlueAdapterV3.setSkimRecipient, (newRecipient)));

        vm.expectRevert(IBlueAdapterV3.TimelockNotExpired.selector);
        adapter.setSkimRecipient(newRecipient);
    }
    ...
    function testSetSkimRecipient(address newRecipient, uint256 timelockDuration) public {
        ...
        vm.expectEmit();
        emit IBlueAdapterV3.SetSkimRecipient(newRecipient);
        adapter.setSkimRecipient(newRecipient);
    ...
    function testSkim(uint256 assets) public {
        ...
        vm.expectEmit();
        emit IBlueAdapterV3.Skim(address(token), assets);
        vm.prank(recipient);
    ...
    function testSubmitBurnSharesNotAuthorized(address caller, bytes32 _marketId) public {
        ...
        vm.prank(caller);
        vm.expectRevert(IBlueAdapterV3.Unauthorized.selector);
        adapter.submit(abi.encode(IBlueAdapterV3.burnShares.selector, abi.encode(_marketId)));
    }

    function testSubmitBurnSharesAlreadyPending(bytes32 _marketId) public {
        vm.prank(curator);
        adapter.submit(abi.encodeCall(IBlueAdapterV3.burnShares, (_marketId)));

        vm.expectRevert(IBlueAdapterV3.DataAlreadyPending.selector);
        vm.prank(curator);
        adapter.submit(abi.encodeCall(IBlueAdapterV3.burnShares, (_marketId)));
    }
    ...
    function testSubmitBurnShares(bytes32 _marketId, uint256 timelockDuration) public {
        ...
        vm.prank(curator);
        adapter.submit(
            abi.encodeCall(IBlueAdapterV3.increaseTimelock, (IBlueAdapterV3.burnShares.selector, timelockDuration))
        );
        adapter.increaseTimelock(IBlueAdapterV3.burnShares.selector, timelockDuration);
        vm.expectEmit();
        emit IBlueAdapterV3.Submit(
            IBlueAdapterV3.burnShares.selector,
            abi.encodeCall(IBlueAdapterV3.burnShares, (_marketId)),
            block.timestamp + adapter.timelock(IBlueAdapterV3.burnShares.selector)
        );
        ...
        assertEq(
            adapter.executableAt(abi.encodeCall(IBlueAdapterV3.burnShares, (_marketId))),
            block.timestamp + adapter.timelock(IBlueAdapterV3.burnShares.selector)
        );
    }
    ...
    function testRevokeBurnSharesNotAuthorized(address caller, bytes32 _marketId) public {
        ...
        vm.prank(caller);
        vm.expectRevert(IBlueAdapterV3.Unauthorized.selector);
        adapter.revoke(abi.encodeCall(IBlueAdapterV3.burnShares, (_marketId)));
    }
    ...
    function testRevokeBurnShares(bytes32 _marketId) public {
        ...
        vm.prank(curator);
        vm.expectEmit();
        emit IBlueAdapterV3.Revoke(
            curator, IBlueAdapterV3.burnShares.selector, abi.encodeCall(IBlueAdapterV3.burnShares, (_marketId))
        );
        adapter.revoke(abi.encodeCall(IBlueAdapterV3.burnShares, (_marketId)));

        assertEq(adapter.executableAt(abi.encodeCall(IBlueAdapterV3.burnShares, (_marketId))), 0);
        ...
        vm.expectEmit();
        emit IBlueAdapterV3.Revoke(
            sentinel, IBlueAdapterV3.burnShares.selector, abi.encodeCall(IBlueAdapterV3.burnShares, (_marketId))
        );
        adapter.revoke(abi.encodeCall(IBlueAdapterV3.burnShares, (_marketId)));
    ...
    function testBurnShares(uint256 timelockDuration, uint256 extraSkip) public {
        ...
        vm.prank(curator);
        adapter.submit(abi.encodeCall(IBlueAdapterV3.burnShares, (marketId)));

        skip(timelockDuration + bound(extraSkip, 0, 3650 days));

        vm.expectEmit();
        emit IBlueAdapterV3.BurnShares(marketId, supplyShares);
        ...
        assertEq(adapter.executableAt(abi.encodeCall(IBlueAdapterV3.burnShares, (marketId))), 0, "executable at");
        assertEq(adapter.realAssets(), 0, "realAssets");
    }

    function testAbdicated() public {
        vm.prank(curator);
        adapter.submit(abi.encodeCall(IBlueAdapterV3.abdicate, (IBlueAdapterV3.burnShares.selector)));
        adapter.abdicate(IBlueAdapterV3.burnShares.selector);

        vm.prank(curator);
        adapter.submit(abi.encodeCall(IBlueAdapterV3.burnShares, (marketId)));

        vm.expectRevert(IBlueAdapterV3.Abdicated.selector);
        vm.prank(curator);
        adapter.burnShares(marketId);
    }
}
