...
// Assume that the vault only uses market V1 adapter, and that it is properly deployed to point to market V1.
rule cantSendAssetsAndCantReceiveAssets(env e, method f, calldataarg args, address user) filtered {
    ...
    require (user != MorphoMarketV1Adapter &&
             ...
             user != currentContract, "do not check if the vault, the adapters themselves, Morpho markets V1 are properly gated to not send/receive assets");

    require (forall address adapter. currentContract.isAdapter[adapter] => (adapter == MorphoMarketV1Adapter), "assume that, currently, only MorphoMarketV1Adapter is under scrutiny");
