...
methods {
    ...
    function _.supply(MorphoMarketV1AdapterV2.MarketParams marketParams, uint256 assets, uint256 shares, address onBehalf, bytes data) external
        => summaryMorphoMarketV1Supply(marketParams, assets, shares, onBehalf, data) expect (uint256, uint256) ALL;
    function _.withdraw(MorphoMarketV1AdapterV2.MarketParams marketParams, uint256 assets, uint256 shares, address onBehalf, address receiver) external
...
function summaryMorphoMarketV1Supply(MorphoMarketV1AdapterV2.MarketParams marketParams, uint256 assets, uint256 shares, address onBehalf, bytes data) returns (uint256, uint256) {
    assert shares == 0;
    assert data.length == 0;
    uint256 returnedShares;
    ERC20Helper.safeTransferFrom(marketParams.loanToken, onBehalf, MorphoMarketV1AdapterV2.morpho, assets);
    return (assets, returnedShares);
}

function summaryMorphoMarketV1Withdraw(MorphoMarketV1AdapterV2.MarketParams marketParams, uint256 assets, uint256 shares, address onBehalf, address receiver) returns (uint256, uint256) {
    assert shares == 0;
    uint256 returnedShares;
    ERC20Helper.safeTransferFrom(marketParams.loanToken, MorphoMarketV1AdapterV2.morpho, onBehalf, assets);
    return (assets, returnedShares);
}
...
function summaryMorphoVaultV1Deposit(uint256 assets, address receiver) returns uint256 {
    ...
    ERC20Helper.safeTransferFrom(currentContract.asset, MorphoVaultV1Adapter.morphoVaultV1, MorphoMarketV1AdapterV2.morpho, assets);
    return shares;
}

function summaryMorphoVaultV1Withdraw(uint256 assets, address receiver, address owner) returns uint256 {
    uint256 shares;
    ERC20Helper.safeTransferFrom(currentContract.asset, MorphoMarketV1AdapterV2.morpho, MorphoVaultV1Adapter.morphoVaultV1, assets);
...
rule cantReceiveShares(env e, method f, calldataarg args, address user) filtered {
    ...
    require(MorphoMarketV1AdapterV2 == 0x10, "ack");
...
rule cantSendAssetsAndCantReceiveAssets(env e, method f, calldataarg args, address user) filtered {
    f -> f.selector != sig:MorphoMarketV1AdapterV2.skim(address).selector &&
    ...
    require (user != MorphoMarketV1AdapterV2 &&
             user != MorphoVaultV1Adapter &&
             user != MorphoMarketV1AdapterV2.morpho &&
    ...
    require (forall address adapter. currentContract.isAdapter[adapter] => (adapter == MorphoMarketV1AdapterV2 || adapter == MorphoVaultV1Adapter), "assume that, currently, only MorphoMarketV1AdapterV2 and MorphoVaultV1Adapter are under scrutiny");
