...
methods {
    ...
    function Utils.encodeMarketParams(BlueAdapterV3.MarketParams) external returns (bytes) envfree;
    function Utils.id(BlueAdapterV3.MarketParams) external returns (BlueAdapterV3.Id) envfree;
    function BlueAdapterV3.allocation(BlueAdapterV3.MarketParams) external returns (uint256) envfree;
...
function summaryWithdraw(BlueAdapterV3.MarketParams marketParams, uint256 assets, uint256 shares, address onBehalf, address receiver) returns (uint256, uint256) {
    uint256 assetsWithdrawn;
    uint256 sharesWithdrawn;
    BlueAdapterV3.Id marketId = Utils.id(marketParams);
    require sharesWithdrawn <= BlueAdapterV3.supplyShares(marketId), "internal accounting of shares is less than actual held shares";
...
rule sentinelCanDeallocate(env e, address adapter, bytes data, uint256 assets) {
    ...
    BlueAdapterV3.MarketParams marketParams;
    require marketParams.loanToken == BlueAdapterV3.asset(), "setup call to have the correct loan token";
