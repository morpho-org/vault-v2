...
methods {
    ...
    function _.accrueInterest(BlueAdapterV3.MarketParams) external => ignoredCallVoidSummary() expect void;

    // Assume that adapters are either BlueAdapterV3 or MorphoVaultV1Adapter.
    ...
    function _.supply(BlueAdapterV3.MarketParams, uint256, uint256, address, bytes) external => ignoredCallUintPairSummary() expect(uint256, uint256);
    function _.withdraw(BlueAdapterV3.MarketParams, uint256, uint256, address, address) external => ignoredCallUintPairSummary() expect(uint256, uint256);
...
hook CALL(uint256 g, address addr, uint256 value, uint256 argsOffset, uint256 argsLength, uint256 retOffset, uint256 retLength) uint256 rc {
    ...
    if (ignoredCall || addr == currentContract) {
        ignoredCall = false;
    } else if (addr == BlueAdapterV3 || addr == MorphoVaultV1Adapter) {
