...
function summaryAllocate(env e, bytes data, uint256 assets, bytes4 selector, address sender) returns (bytes32[], int256) {
    ...
    // Assume length 3 for simplicity. This covers MorphoMarketV1AdapterV2 and the rule similarly holds for MorphoVaultV1Adapter with ids.length == 1.
    ...
    require currentContract.firstTotalAssets() < 2 ^ 20 * 2 ^ 128, "Blue fits total supply assets on 128 bits, and assume at most 2^20 markets";
...
function summaryDeallocate(env e, bytes data, uint256 assets, bytes4 selector, address sender) returns (bytes32[], int256) {
    ...
    // assume MorphoMarketV1AdapterV2. The rule similarly holds for MorphoVaultV1Adapter with ids.length == 1.
    require ids.length == 3, "for simplicity, assume a fixed number of ids";
